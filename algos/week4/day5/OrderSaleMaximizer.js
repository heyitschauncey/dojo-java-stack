...
class OrderSaleMaximizer {
  ...
  insertOrders(orders) {
    ...
    // return the amount of total orders
  ...
  processMatchingOrders() {
    ...
    while (minSell && maxBuy && minSell.price <= maxBuy.price) {
      // make some boolean variables to make it easy to see the logic
      ...
      if (sameQuantity) {
        // if the same quantity then both orders need to be removed from the heaps
        ...
        // if fewerSells, then remove the sell order from the heap
        this.total += minSell.price * minSell.quantity;
        ...
        // if fewerBuys, then remove the buy order from the heap
